def change_memoization(amount, coins) :
    ...
    def calc(a, pos):
        if amount==a:
            return 1
        if amount<a or pos>=len(coins):
            return 0
        if (a,pos) not in dp:
            dp[(a,pos)]=calc(a+coins[pos],pos)+calc(a,pos+1)
        return dp[(a, pos)]
    return calc(0, 0)
...
def change_tabulation(amount, coins) :
    ...
    for target in range(1,amount+1):
        for i in range(len(coins)-1,-1,-1):
            ...
            if coins[i]<=target:
                dp[target][i]+=dp[target-coins[i]][i]
    return dp[amount][0]
def change_tabulation_space_optimized(amount,coins):
    dp=[0]*(amount+1)
    dp[0]=1
    for i in range(len(coins)-1,-1,-1):
        temp=[0]*(amount+1)
        ...
        for target in range(1,amount+1):
            ...
            if coins[i]<=target:
                temp[target]+=temp[target-coins[i]]
        dp=temp
    return dp[amount]
if __name__ == '__main__':
    ...
    # amount=5
    # coins=[5,2,1]
    print(change_tabulation_space_optimized(amount,coins))